...
const ReportContainer = () => {
    ...
    return (
        <Layout>
            <div>
                <ReportCateInput postInput={postInput} postInputHandle={postInputHandle} level={data?.level} />
                <div
                    className="outline-0 mb-[8px] w-full h-[100%] min-h-[52px] rounded-md text-start py-[16px] px-[24px] text-sm bg-white text-bb22 overflow-y-auto break-all"
                >
                    {
                        contentData?.data !== undefined && (
                            contentData?.level === 1 && (contentData?.data.content !== undefined &&
                    ...
                    {
                        contentData?.data !== undefined && (
                            contentData?.level === 2 && (contentData?.data.comment !== undefined &&
                                postInput.category === "1" ?
                                contentData?.data.accountName :
                                postInput.category === "2" ?
                                    <span className='text-bb88'>신고대상이 아닙니다.</span> :
